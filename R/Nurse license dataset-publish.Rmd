...
fl_state <- fl_state %>%
  filter (!str_detect(app_type, "^Initial Temporary")) %>% #Removes 16 Florida temps)
...
#Compare columns to see what we can remove and rename.
...
test_join <- inner_join (all_issued, all_apps, by="FILE_NBR") %>%
...
test_join2 <- test_join %>%
  select (LICENSEE_NAME.x, FILE_NBR, issue_date, application_date, process_time )
...
* 1.66M records (including pending)
...
```{r} 
#How many still-pending applications are in full data? >> #96,173 
valid_data %>% 
  filter (str_detect (year, "^Pending")) %>%
  nrow()

#Remove any applications after Sept. 23, 2021 >> 68848
valid_data2 <- valid_data2 %>%
  filter (application_date <= as.Date("2021-09-23")) 
nrow(valid_data)-nrow(valid_data2)

#How many still-pending applications who applied before Sept. 23 are in this filtered data? >> #70,045 
valid_data2 %>% 
  filter (str_detect (year, "^Pending")) %>%
  nrow()
...
#removes 96357 total records, to end up with 1,548,855 records. Some overlap if nurses applied and were issued after Sept. 23)
nrow(valid_data)-nrow(valid_data3)

#How many still-pending applications who applied before Sept. 23 remain in this filtered data? >> #70,045, so none were lost
valid_data3 %>% 
  filter (str_detect (year, "^Pending")) %>%
  nrow()
```
...
## Write to file
Includes 2019-2021 and still-pending applications submitted after July 2020.
1.65 million rows x 44 columns, so will be a very large file.
```{r}
#write_csv(valid_data2, "../output/nurse_licenses_all_2019_2021.csv")
```
...
* Only includes applications submitted after July 2020 that were still pending when their state boards submitted records to NPR.
* 21 states included pending applicants = 70,045 applications.
...
Adding Florida, increases to 253,615
```{r}
...
* Adding Florida increases count to at least 167,536 nurses applying by endorsement in 2021.
```{r}
