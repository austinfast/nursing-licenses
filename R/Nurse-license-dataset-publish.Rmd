---
title: "How long does it take to license nurses?"
...
1. Data set of licenses issued Jan. 1 - Sept. 23, 2021 *(see "Write to file" section below)*
    + Too large for GitHub -- create by running this script using data in state-data folder.
2. Data set of pending applications submitted Jan. 1 - Sept. 23, 2021 *(see "Write to file" section below)*
    + Create by running this script using data in state-data folder.
3. State summary table (Jan. 1 - Sept. 23, 2021): *Includes mean, median, count for each state, plus national rates, minimums and maximums for each type.*
    + [Wide version](https://github.com/austinfast/nursing-licenses/blob/main/output/2021_state_summary.csv)
    + [Tidy version](https://github.com/austinfast/nursing-licenses/blob/main/output/2021_state_summary_tidy.csv)
4. Summary split by license type and application type
    + [State averages](https://github.com/austinfast/nursing-licenses/blob/main/output/state_averages_by_app_lic_type.csv)
    + [National averages](https://github.com/austinfast/nursing-licenses/blob/main/output/natl_averages_by_app_lic_type.csv)
    
**The following summary tables include only new RN and LPN licenses, who applied by:**

  * **Endorsement** (licensed nurses re-applying in a new state); 
  * **Exam** (typically new graduates taking the national nursing exam to get licensed); 
  * **Other** (includes 3,055 records listed as "Foreign Applicant", "Inactive", "SSL to MSL", "International" and "Military Vet Expedited Initial Licensure" [Florida only]); 
  * **Unknown** (39,703 records, including all of Connecticut and Virginia's records plus pending applications in Michigan and a small number of records in Pennsylvania and Colorado).
  * Excludes temporary/provisional licenses, renewals, reinstatements, and exam-retest applications.

5. Summary split by license type only *(RN or LPN)*
    + [State averages](https://github.com/austinfast/nursing-licenses/blob/main/output/state_averages_lic_type_only.csv)
    + [National averages](https://github.com/austinfast/nursing-licenses/blob/main/output/natl_averages_lic_type_only.csv)
6. Summary split by application type only *(new, permanent RN and LPN licenses only, excluding re-tests, reinstatements, reactivations, etc.)*
    + [State averages](https://github.com/austinfast/nursing-licenses/blob/main/output/state_averages_app_type_only.csv)
    + [National averages](https://github.com/austinfast/nursing-licenses/blob/main/output/natl_averages_app_type_only.csv)
7. Summary combining license type and application type *(new, permanent RN and LPN licenses only, excluding re-tests, reinstatements, reactivations, etc.)*
    + [State averages](https://github.com/austinfast/nursing-licenses/blob/main/output/state_averages_combined.csv)
    + [National averages](https://github.com/austinfast/nursing-licenses/blob/main/output/natl_averages_combined.csv)
...
* Arkansas
* California-RN & LPN boards *(provided anonymized records - charged $60.17)*
* Colorado 
* Connecticut
...
* Florida: Claimed no responsive records exist in December 2021. When NPR pointed out annual reports advertise license processing times, the Florida Department of Health charged NPR $150, but did not provide records to NPR by the publication date of March 10, 2022.
  + NPR received records on March 15, 2022, and has updated this document to reflect their addition.
...
* Florida's Department of Health provided these records on March 15, 2022, after publication.
* Includes Endorsement, Exam, Re-exam application, Initial Temporary Military - RN, and Military Vet Expedited Initial Licensure
* No temporary licenses
* Includes pending applications 
* Verify licenses at https://floridasnursing.gov/licensing/.
...
### Confirm FL DOH matched correctly
...
### Florida's pending applications

Which applications are still pending? 

* Remove applications whose file_nbr and tran_desc shows up in fl_state as issued.
* File numbers are not consistent from all_apps to fl_state for some reason? 
```{r cache=TRUE}
open_apps <- all_apps %>% 
...
* 1.70M records (including pending)
* 1.55M without pending
```{r}
all_data2 %>% 
  filter (process_time2 > -1) %>% 
...
# Write to file
Includes 2019-2021 and still-pending applications submitted after Dec. 31, 2020.
1.66 million rows x 43 columns, so will be a very large file.
```{r}
...
**Adding Florida**
* Increases to 126,193 licensed by endorsement in 2021; 
* 46,868 still-pending (Jan 2021-Dec 2021)
* At least 173,061 nurses applied by endorsement in 2021.
```{r}
...
Combined RNs+LPNs and combine application type, keeping only exam, endorsement, other, unknown app types
THIS ALLOWS FOR CT & VA TO BE ADDED IN
```{r}
#Analyzing by license type only
