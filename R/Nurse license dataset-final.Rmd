...
```{r}
state <- read_excel("State data/NC-NPRApplicationData.xlsx") %>%
  clean_names() 

#No records have both initial exam and initial endorsement, so can combine them into issue_date, which is empty
state %>%
...
state2 <- state %>%
  mutate (issue_date = as.Date(coalesce(initial_exam, initial_endorsement)),
          application_date = as.Date(format(app_sumbit_date, "%Y-%m-%d")),
          last_renewal=as.Date(as.numeric(last_renewal), origin =  "1899-12-30")) #%>% #Converts last renewal to date format

#Renewals are messy, so not going to include
state3 <- state2 %>%
  mutate (process_time = case_when (
    app_type %in% c("Exam", "Endorsement") ~ issue_date - application_date,
...
jones <- state2 %>% filter (first_name=="Kimberly" & middle_name=="Ann" & last_name=="Jones")
...
#Some nurses applied multiple times. There are 33,815 individuals - 2,079 have more than 1 record, meaning they re-tested or submitted multiple endorsement applications >> 1768 are exams + 311 are endorsement
nc_multiples <- state3 %>%
  group_by (first_name, middle_name, last_name) %>%
  mutate (count=n()) %>%
  arrange (desc(count), last_name, first_name, city, application_date)

nc_multiples %>%
  ungroup() %>%
  distinct (first_name, middle_name, last_name, .keep_all=T) %>%
  filter (count > 1) %>%
  count (app_type)
  #count (count)
...
nc_multiples2 <- nc_multiples %>%
  filter (count > 1) %>%
  slice_tail()
#Create dataframe with all 1-record people (31736 people)
nc_multiples1 <- nc_multiples %>%
  filter (count==1)
#Combine 1-record people with only 1 record from the 2079 multiple-record people
...
183 column names from 31 states pared down to 45 columns
```{r}
#Compare columns to know which ones to remove from each state dataframe in code above
all_data <- compare_df_cols (ar_state, ca_state, co_state, ct_state, hi_state, ia_state, il_state, in_state, ky_state, la_state, ma_state, me_state, mi_state, mn_state, ms_state, mt_state, ne_state, nh_state, nj_state, nm_state, nc_state, oh_state, ok_state, or_state, pa_state, sc_state, tx_state, vt_state, va_state, wv_state, wy_state)
...
all_data2 <- bind_rows (ar_state, ca_state, co_state, ct_state, hi_state, ia_state, il_state, in_state, ky_state, la_state, ma_state, me_state, mn_state, mi_state, mn_state, ms_state, mt_state, ne_state, nh_state, nj_state, nm_state, nc_state, oh_state, ok_state, or_state, pa_state, sc_state, tx_state, vt_state, va_state, wv_state, wy_state) %>%
  ...
  mutate (name = case_when (
    ...
    process_time2 = if_else (data_state %in% c("AR", "IL", "KY", "ME", "MN", "MS", "NC", "OR", "SC", "TX", "VA", "WY"), process_time, process_time2)) %>% 
...
Calculate averages for 31 states that provided usable data
```{r}
#1,521,346 records (without CA renewals)
...
#624,597 records removed (NA + negative)
...
In these 31 states, over half (58%) the 219,000 RNs and LPNs issued new licenses Jan. 1-Sept. 23, 2021, waited more than two months (126,006 nurses). Over one-third  (36%) of nurses (78,681) waited more than 3 months to get a new license issued in 2021.
...
exam_end %>%
  ...
  theme(axis.text.x = element_text(angle=60, hjust=1))
```
Export graphs of means/medians
```{r}
lic_types <- c("RN", "LPN")
...
write_csv (final2, "FOR CONNIE-State-summary-lic-type-app-type2.csv")
```
...
How many new licenses in 2021? ##219052 (excluding re-tests [which skew medians longer], reinstatements, reactivations, etc.)
```{r}
avg_counts %>%
  ungroup() %>%
...
ca_weeks_to_license4 <- ca_weeks_to_license3 %>%
  ungroup() %>%
...
rn_graph <- ca_weeks_to_license4 %>%  
  ...
  filter (lic_type %in% c("LPN", "RN")) %>% #"LPN", 
  filter (standard_app_type %in% c("Exam", "Endorsement")) %>%
...
ggsave(plot = rn_graph,
       file = paste0("/Users/austinfast/Documents/NPR/210908-nursing licenses/Graphs/CA-RN-JanSept21-deficient-app-type.png"),
       width = 6,
       height = 4
       )
...
ca_weeks_to_license5 <- ca_weeks_to_license2 %>%
  ...
  mutate (pct = count / sum(count) * 100,
          ...
          mean_weeks = ceiling (as.numeric(mean21/7)))
...
write_csv (ca_deficient, "Nurse licenses-weeks to licensure-CA-deficient-complete.csv")
```
...
states <- str_to_upper(c("ar", "co", "hi", "ia", "il", "in", "ky", "ma", "me", "mi", "mn", "ms", "mt", "ne", "nh", "nj", "nm", "oh", "ok", "or", "pa", "sc", "tx", "vt", "LA", "WY", "WV", "CT", "VA", "NC", "CA"))
