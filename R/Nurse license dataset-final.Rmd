...
Only two temporary included in duration field, contains mixed endorsement/exam for some reason
```{r}
...
#Keep only Exam/Endorsement and cut out repeat redcords
tn_state <- tn_state %>% 
  filter (app_type %in% c("Exam", "Endorsement"))  %>%
...
```{r}
#1,565,686 records (without CA renewals)
...
#Removes exam-retest, reinstatements, renewals
#keeps only 2021
state_avg_lic_type_only_lumped <- valid_data2 %>%
  filter (lic_type %in% c("LPN", "RN")) %>%
  filter (standard_app_type %in% c("Endorsement", "Exam", "Other", "Unknown")) %>%
  filter (year == "2021") %>%
...
In these 32 states, over half (57%) the 227,000 RNs and LPNs issued new licenses Jan. 1-Sept. 23, 2021, waited more than two months (129,616 nurses). Over one-third  (35%) of nurses (80,143) waited more than 3 months to get a new license issued in 2021.
...
```{r}
ca_averages <- state_averages %>%
  filter (data_state=="PA") %>%
  filter (year != "2019")

#write_csv (ca_averages, "ca_averages.csv")
...
Approx 63% of nurse licenses nationally, using counts of active licenses as a proxy for new nurse applications
* Getting Florida would add 6%, and getting TN would add 2%
...
```{r}
nnd <- read_excel ("Aggregate-AllActiveLicensesExcel-112321.xls", skip=3) %>%
  select(1,4,7) %>%
...
nnd3 <- nnd2 %>%
  mutate (data = if_else (state %in% c("ar", "co", "ct", "hi", "ia", "il", "in", "ky", "la-pn", "ma", "me", "mi", "mn", "ms", "mt", "ne", "nc", "nh", "nj", "nm", "oh", "ok", "or", "pa", "sc", "tx", "vt", "va", "wv-rn", "wy", "ca-rn", "ca-pn"), "received", "missing"), #, "fl", "tn"
...
Export graphs of means/medians -- uses exam_end at line 4036 (before Connie Jin version)
```{r}
lic_types <- c("RN", "LPN")
...
exam_end2 <- exam_end %>%
  ...
  pivot_longer (cols = c("mean", "median", "count", "count_over30days", "pct_over30days", 
                         ...
                         "count_over1year", "pct_over1year"),  #, "count_2020", "count_2021", "change_med"
               names_to = "type", 
               values_to = "value")
...
exam_end4 <- exam_end3 %>%
  ...
  select (1:3, natl_look, state_min, state_max, 4:32)
...
exam_end_lic_only2 <- exam_end_lic_only %>%
  ...
  mutate (standard_app_type = "All new licenses") %>%
  select (1:2, standard_app_type, natl_look, state_min, state_max, 3:32)
...
write_csv (final2, "FOR CONNIE-State-summary-lic-type-app-type3+NC+TN.csv")
```
...
How many new licenses in 2021? ##227357 (excluding re-tests [which skew medians longer], reinstatements, reactivations, etc.)
```{r}
avg_counts %>%
  ungroup() %>%
...
ca_weeks_to_license2 %>% 
  ungroup() %>%
  count (year)
#39121 pending apps filed in 2021
ca_weeks_to_license2 %>% 
  ungroup() %>%
  filter (year=="Pending") %>%
...
ca_weeks_to_license4 <- ca_weeks_to_license2 %>% #change to 3 if want to take only Jan-Sept 2021
  ungroup() %>%
...
ca_weeks_to_license5 <- ca_weeks_to_license2 %>%
  ungroup() %>%
  filter (year == "2021") %>%
  mutate (deficient = if_else (is.na(deficient), "complete", deficient)) %>%
  ...
  mutate (pct = count / sum(count) * 100,
          med_weeks =  (as.numeric(median21/7)),#ceiling
          mean_weeks =  (as.numeric(mean21/7)))#ceiling
...
#  filter (standard_app_type == "Exam") %>%
 # filter (!(standard_app_type %in% c("Reinstatement", "Renewal", "Exam-retest"))) %>%
  ...
  facet_wrap (~ deficient, ncol=1, scales='free', 
              labeller = label_wrap_gen(multi_line=FALSE)) + 
 # scale_x_continuous(breaks=c(0,30, 60, 90, 120)) +
  scale_x_continuous(breaks=seq(0, 53, by = (365 / 7 / 12)),
                     labels=as.character(seq(0, 12, by = 1)),
                     limits=c(-0.5, 53)) +
  #Add padding so they appear equal width
  scale_y_continuous(breaks=seq(0,1500, by = 200),#900 Jan-sept #1500 fullyear
                     limits=c(0,1500)
                     #labels = function(x) stringr::str_pad(x, width = 4, pad = "0")
                     ) +
  #Add median lines
