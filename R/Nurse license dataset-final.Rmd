---
title: "Nurse licensing dataset-31 states"
author: "Austin Fast"
date: "02/14/2022"
...
#setwd('/Users/austinfast/Documents/NPR/210908-nursing licenses')
start_time <- Sys.time()

#Austin: Before running, find&replace /Users/austinfast/Documents/NPR/210908-nursing licenses/State data/ with /Users/austinfast/Documents/NPR/210908-nursing licenses//Users/austinfast/Documents/NPR/210908-nursing licenses/State data/
```

Import Arkansas
* Includes Endorsement, Exam, International (3), Renewal (200), Retest (600)
* Includes temporary (split by endorsement and examination)
* No pending applications

Arkansas doubled names, one temp permit and one permanent license. 
```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/AR-Application Management for FOIA 09_23_2021.xlsx", skip=1) %>% 
  clean_names()
...
Import California - anonymized, providing an entity_id to identify each individual. 

VNs by exam progress through three stages: Exam Request, Exam Results, Initial
  See MILLER, SYDNEY NICOLE for proof at https://search.dca.ca.gov/details/4301/VN/704727/97585d1c86321b5b232e49d5c7a65d38
  Only VN license from AGUANGA, CA issued in 2019-2021. 
  Initial date approved matches her issuance date (May 23, 2019)

VNs by endorsement process from: Endorsement to Initial
  See CERDA, CHRYSTAL E of Yermo, CA (entity number: 4016139) Initial date matches (May 21, 2019)

RNs by exam progress through two stages: Request for RN Exam, Apprvd Exm App - Pendng Exm Pass Rslts
  See STEELE, ALEXIS ANNE LOUISE for proof at 
  https://search.dca.ca.gov/details/4001/RN/95252157/31b623dd5e52fbc5ee0778f0edaa5cac
  Only RN from ALTAVILLE, CA issued 2019-2021
  Apprvd Exm App - Pendng Exm Pass Rslts date approved matches her issuance date (July 8, 2021)

Each entity_id has multiple rows, one for each stage of their application process. This code creates one row for each person, so we can calculate total processing time from the start of stage one until the license was issued. 

PROCESS CA's VOCATIONAL NURSES FIRST
```{r}
state_vn <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/CA-BVNPT PRA Report 2022-01-04.xlsx", sheet=1) %>%
  clean_names()
...
PROCESS CALIFORNIA RNs
```{r}
state_rn <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/CA-BRN PRA Report 2022-01-04.xlsx", sheet=1) %>%
  clean_names()
...
#====== PROCESS RNS BY ENDORSEMENT
#Pull out endorsement applicants, which only have one stage in CA's records
#65662 endorsement individuals (including expired, withdrawn, denied, canceled)
rn_endorse <- rns3 %>%
...
#====== PROCESS RN TEMPORARY LICENSES
#Emergency RN Temp Licenses issued since Jan. 29 to Nov. 3, 2021
#RN Request for Temporary License	issued throughout whole period
rns_temp <- state_rn %>%
...
#JOIN ENDORSEMENT, EXAM AND TEMP DATAFRAMES
...
#summarize all types together for Jackie Fortier at KPCC
for_jackie <- ca_state %>%
  filter (app_type != "Exam-retest" & lic_type %in% c("RN", "LPN")) %>%
  group_by (year, lic_type, app_type, deficient) %>%
  #group_by (year, deficient) %>%
  summarize (mean = round(mean(process_time2, na.rm=T)),
             median = median (process_time2),
             count = n(),
             over30days = sum(process_time2 > 30),
  ...
  arrange (lic_type, app_type, deficient, year) %>%
  filter (year %in% c("2021", "2020")) %>%
  select (year, lic_type, app_type, deficient, median, count, pct, 8:19)

#write_csv (for_jackie, "2020-2021-CA-comparison.csv")
...
Import Colorado - combines renewal/new dates 
* includes Endorsement, Exam, Temp (mixed), 15 NAs
* includes Pending applications
* no renewals

```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/CO-250-2021_RN_TRN_PN_TPN_Applicants.xlsx") %>% clean_names()
...
* Includes pending
Cannot split by exam, endorsement, etc.
```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/CT-NPR FOI Austin Fast - RN, LPN - Update-clean.xlsx") %>%
  clean_names() %>%
...
```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/HI-RN_LPN Applications Report-2021-11-19-18-32-54.xlsx", skip=9) %>%
  clean_names() %>%
...
* No renewals
```{r}
il1 <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/IL-Responsive Documents/2019 Nurses Applied and Issued by Endorsement.xls.xlsx", skip=8) %>% 
  clean_names()  %>%
  mutate (application_type = "Endorsement",
          ...
          license_type = case_when (
            ...
            TRUE ~ "PROBLEM"))
il2 <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/IL-Responsive Documents/2019 Nurses Applied and Issued by Exam.xls.xlsx", skip=8) %>% 
  clean_names() %>%
...
il3 <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/IL-Responsive Documents/2020 Nurses Applied and Issued by Endorsement.xls.xlsx", skip=8) %>% 
  clean_names()  %>%
  mutate (application_type = "Endorsement",
          app_year = 2020,
          license_type = case_when (
            ...
            str_detect(license_number, "^43") ~ "LPN",
            TRUE ~ "PROBLEM"))
il4 <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/IL-Responsive Documents/2020 Nurses Applied and Issued by Exam.xls.xlsx", skip=8) %>% 
  clean_names() %>%
  mutate (application_type = "Exam",
          app_year = 2020,
          license_type = case_when (
...
il5 <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/IL-Responsive Documents/2021 Nurses Applied and Issued by Endorsement.xls.xlsx", skip=8) %>% 
  clean_names()  %>%
  mutate (application_type = "Endorsement",
          app_year = 2021,
          license_type = case_when (
            ...
            str_detect(license_number, "^43") ~ "LPN",
            TRUE ~ "PROBLEM"))
il6 <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/IL-Responsive Documents/2021 Nurses Applied and Issued by Exam.xls.xlsx", skip=8) %>% 
  clean_names() %>%
  mutate (application_type = "Exam",
          app_year = 2021,
          license_type = case_when (
...
Import Iowa - includes Exam, Endorsement, Re-Exam, Reactivation, Renewal
Includes pending

Duration field is not useful -- contains almost all permanent, only 2 temporary licenses, plus  803 endorsement and 791 Examination for some reason
```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/IA-Records Request.xlsx") %>% clean_names()
...
Import Indiana
Includes Exam, Endorsement. Only includes pending/denied reinstatements (232) & renewals (14).
Includes 3959 temp licenses.
Includes pending
```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/IN-RN and LPN Applications Received 1.1.19-10.6.21.xls", 
                    skip=1,
...
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/KY/KY-NPR_ORR_20211109.xlsx",
...
```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/LA-NPR Public Records Request.LSBPNE.xlsx",
...
#Every person has a match after removing duplicates, so should be OK
anti_join (la_state, la_state2, by = c("first_name", "middle_name", "last_name", "city", "lic_type", "app_type", "process_time2")) %>% View
...
Import Massachusetts - little tricky because I had to match application dates with license dates by their name rather than an ID number
Includes Exam, Reciprocity, Covid-19 emergency order
Includes pending

```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MA/Copy of Application Status Report - Nursing 9.23.21.xlsx") %>% clean_names()
str(state)
...
ma_rns <- read_csv ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MA/MA_RN_data_101921_date.csv", 
                    col_types = cols(.default = "c", expiration_date = "D", issue_date = "D")) 
ma_rns_tmp <- read_csv ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MA/MA_RNtemp_data_101921.csv", 
                    col_types = cols(.default = "c", expiration_date = "D", issue_date = "D"))
ma_lpns <- read_csv ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MA/MA_LPN_data_101921.csv",
                    col_types = cols(.default = "c", expiration_date = "D", issue_date = "D"))
ma_lpns_tmp <- read_csv ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MA/MA_LPNtemp_data_101921.csv", 
                    col_types = cols(.default = "c", expiration_date = "D", issue_date = "D"))
...
Includes pending
```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/ME/Days to Licensure Merge.xlsx",
                    #col_types = c("date", "text", "text", "text", "text", "text", "text", "text", "text", "text", "date", "date")
                    ) %>% 
                      clean_names()
...
```{r}
state <- read_csv ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MI-PROD__simple_saved.csv",
...
IMPORT MINNESOTA - only in this markdown document
Includes exam, endorsement, renewals, reinstatement
Includes temporary (endorsement permits)
Includes pending applications
```{r}
sheets <- excel_sheets(path = "/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MN-npr_data_request_v2.xlsx")

state <- tibble()
for (i in sheets){
  #i <- "Q1 21"
  step_df <-read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MN-npr_data_request_v2.xlsx",
                       sheet = i) %>% 
    clean_names() %>%
    mutate (source = i)
...
No pending applications
```{r}
state <-read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MS-NPR Request-correct2.xlsx", skip=2) %>% 
    clean_names() 
...
mt_lpn1 <- read_excel ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MT/MT-LPN Licenses - Exam.xlsx") %>% 
  clean_names() %>%
  mutate (app_type = "Exam",
          lic_type = "LPN")
mt_lpn2 <- read_excel ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MT/MT-LPN Licenses - Credential.xlsx") %>% 
  clean_names() %>%
  mutate (app_type = "Endorsement",
          lic_type = "LPN")
...
mt_lpn3 <- read_excel ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MT/MT-LPN Applications - Exam .xlsx") %>% 
  clean_names() %>%
  mutate (app_type = "Exam",
          lic_type = "LPN")
mt_lpn4 <- read_excel ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MT/MT-LPN Applications - Credential .xlsx") %>% 
  clean_names() %>%
  mutate (app_type = "Endorsement",
          lic_type = "LPN")
...
mt_rn1 <- read_excel ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MT/MT-RN Licenses - Exam.xlsx") %>% 
  clean_names() %>%
  mutate (app_type = "Exam",
          lic_type = "RN")
mt_rn2 <- read_excel ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MT/MT-RN Licenses - Credential.xlsx") %>% 
  clean_names() %>%
  mutate (app_type = "Endorsement",
          lic_type = "RN")
...
mt_rn3 <- read_excel ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MT/MT-RN Applications - Exam.xlsx") %>% 
  clean_names() %>%
  mutate (app_type = "Exam",
          lic_type = "RN") 
mt_rn4 <- read_excel ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/MT/MT-RN Applications - Credential .xlsx") %>% 
  clean_names() %>%
  mutate (app_type = "Endorsement",
          lic_type = "RN") %>%
  select(-c(x9, x10))
...
Import Nebraska
Includes exam, endorsement
Three types of temporary: Grad LPN/Grad RN (emergency Covid order); refresher temporary license + temporary license
Includes pending

RN pre-exam = provisional?
```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/NE-NPR Nursing Data Request-092821.xlsx") %>% clean_names()
...
Includes exam (split into US, foreign, Canada), endorsement, reinstatement, application, comparable education, re-entry
Renewals cannot be calculated (doesn't include application dates)
Includes temporary RN/LPN, some with exam/endorsement information
Includes pending

Includes checklist complete date, but shows license nearly always issued same day checklist complete.
```{r}
state <- read_csv("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/NH-initial-apps.csv")%>%
  clean_names()
...
IMPORT NEW JERSEY DATA
Includes exam, endorsement, foreign applicant, grad license (2020/2021), reactivation, reinstatement; renewals not done correctly
Includes pending

Emergency graduate license order: https://www.njconsumeraffairs.gov/COVID19/Pages/Emergency-Graduate-Licensure.aspx
```{r}
#Import RN files
nj1 <- read_csv ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/NJ-Responsive Documents/clean/NPR - RPN NEW APPLICANTS SINCE 2019 100521.csv") %>% clean_names()
nj2 <- read_csv("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/NJ-Responsive Documents/clean/NPR - RPN GRAD LICENSEES 100521.csv") %>% clean_names() 
nj3 <- read_csv("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/NJ-Responsive Documents/clean/NPR - RPN PENDING REINSTATEMENT APPLICANTS SINCE 2019 100521.csv") %>% clean_names()
nj4 <- read_csv("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/NJ-Responsive Documents/clean/NPR - RPN COMPLETED REINSTATEMENT APPLICANTS SINCE 2019 100521.csv") %>% clean_names()
#Import LPN files
nj_lpn1 <- read_csv ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/NJ-Responsive Documents/clean/NPR - LPN NEW APPLICANTS SINCE 2019 100521.csv") %>% clean_names()
nj_lpn2 <- read_csv("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/NJ-Responsive Documents/clean/NPR - LPN GRAD LICENSEES 100521.csv") %>% clean_names() 
nj_lpn3 <- read_csv("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/NJ-Responsive Documents/clean/NPR - LPN PENDING REINSTATEMENT APPLICANTS SINCE 2019 100521.csv") %>% clean_names()
nj_lpn4 <- read_csv("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/NJ-Responsive Documents/clean/NPR - LPN COMPLETED REINSTATEMENT APPLICANTS SINCE 2019 100521.csv") %>% clean_names()
nj_lpn5 <- read_csv("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/NJ-Responsive Documents/clean/NPR - LPN RENEWALS SINCE 2019 100521.csv",
...
IMPORT NEW MEXICO DATA - Second try after NM resent data
...
Includes renewals
```{r}
state <- read_csv("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/NM-11.03.2021-clean.csv") %>%
  clean_names() 
...
Includes exam, endorsement, competency (renewals?), refresher course, reinstatement, reactivation
Doesn't include pending
Includes renewals, but labelled two different ways, so not going to try and clean
```{r}
#state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses//Users/austinfast/Documents/NPR/210908-nursing licenses/State data/NC-NPRApplicationData.xlsx") %>%
#  clean_names() 

state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/NC-NPRApplicationDataV2.xlsx") %>%
  clean_names() 
...
```{r}
state <- read_csv("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/OH-DAS Report - 9_28_2021-clean.csv",
...
Import Oklahoma data
Includes Exam, endorsement, reinstatement, renewal, retest, SSL to MSL (1482), temporary (68); ALSO 109 "INACTIVE"
Includes pending (from Oct. 1, 2020)

Only has data from Oct. 1, 2020 onward because of earlier data migration
Provided estimates and counts from Jan. 1, 2019-Sept. 30, 2020 instead
...
```{r}
state <- read_csv ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/OK-Copy of NPR Final Data-clean4.csv",
...
Only most recent renewal data is correct
...
```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/OR-NPR chart 10-1-21.xlsx") %>% clean_names()
...
From Rebecca Fuhrman with PA DOS: Above or Below Threshold applies to an application to reactivate or reinstate a license. Below threshold would apply to a reactivation/reinstatement where the license has been expired/inactive/suspended for less than 5 years. Above threshold would apply to reactivation/reinstatement applications where the license has been expired/inactive/suspended for 5 or more years, in which case the continued competency requirements of Sections 21.30a, 21.156a, 21.332a, and 21.725 of the Board's regulations apply. 
```{r}
sheets1 <- excel_sheets(path = "/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/PA/PA-RN APPLICATIONS.xlsx")
sheets2 <- excel_sheets(path = "/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/PA/PA-LPN APPLICATIONS.xlsx")

state <- tibble()
for (i in sheets1){
  #i <- "Q1 21"
  step_df <-read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/PA/PA-RN APPLICATIONS.xlsx", sheet = i) %>% 
    clean_names()
    state <- rbind (state, step_df)
}
for (i in sheets2){
  #i <- "Q1 21"
  step_df <-read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/PA/PA-LPN APPLICATIONS.xlsx", sheet = i) %>% 
    clean_names()
    state <- rbind (state, step_df)
}

#pa_rn <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/PA/PA-RN APPLICATIONS.xlsx") %>% clean_names()
#pa_rn_renew <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/PA/PA-RN RENEWALS.xlsx") %>% clean_names()
#pa_lpn <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/PA/PA-LPN APPLICATIONS.xlsx") %>% clean_names()
#pa_lpn_renew <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/PA/PA-LPN RENEWALS.xlsx") %>% clean_names()
...
No pending
```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/SC-Nursing FOIA Request RESPONSE.xlsx"#, 
                    #skip=1,
...
```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/TN-Updated Nursing Data for NPR 02042022 EM.xlsx",
...
Import Texas
Includes exam, endorsement, retest (all split by US / foreign)
No pending
No renewals (not functional - renewal date not recorded, so all renewals/reactivations have negative processing times)

2021 stats (detailed)
NCLEX-PN Application ñ U.S. Graduates
Endorsement LVN ñ U.S. EDUCATED BUT NOT PUERTO RICO EDUCATED
NCLEX RETEST Application ñ U.S. Graduates
NCLEX-RN Application ñ U.S. Graduates
Endorsement RN ñ U.S. EDUCATED BUT NOT PUERTO RICO EDUCATED
Endorsement RN ñ INTERNATIONAL AND PUERTO RICO EDUCATED
NCLEX Application ñ International Graduates (Including Puerto Rico)
NCLEX RETEST Application ñ U.S. Graduates

```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/TX-Approved Licenses-USETHIS2.xlsx") %>% 
  clean_names()

tx_temp <- read_csv ("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/TX-ApplicationsWithoutLicensure.csv") %>% 
  clean_names()
...
Import Vermont data emailed from Gabriel M. Gilman, General Counsel
Ask about difference between endorsement and "fast track" endorsement -- 45 pending fast track applications

Includes exam, endorsement, fast-track endorsement (pending only), emergency temporary license
Includes pending
No renewals (not correctly)
```{r}
state <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/VT-NPR_Nursing_PRR_09-23-21_2021-09-23_20-10-30.xlsx") %>% 
  clean_names()
...
IMPORT VIRGINIA -- anonymized data
Includes RN, RN-Temp, LPN, LPN-Temp (Not split by exam, endorsement)
No pending
No renewals

```{r}
sheets <- excel_sheets(path = "/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/VA-NPR Request RN LPN 30dayapplicationprocessing Jan 1 2019 -Sep 30 2021.xlsx")

state <- tibble()
for (i in sheets){
  #i <- "Q1 21"
  step_df <-read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/VA-NPR Request RN LPN 30dayapplicationprocessing Jan 1 2019 -Sep 30 2021.xlsx",
                       sheet = i) %>% 
    clean_names() %>%
    mutate (entry_date = as_date(entry_date),
            last_item_complete_date = as_date(last_item_complete_date),
...
#Virginia claims to process 99% of applications within 30 days, but that's 30 days from getting marked complete.
#docs_time records how many days from being marked complete till they were issued. This confirms their claim, but also shows only #24.7% of the applications were issued within 30 calendar days from their submission.
va_summary <- va_state %>% 
  filter (year=="2021") %>%
...
(139 + 1778) / (1024 + 6889) #24.2%, standardizing with other states
...
Import West Virginia data (RNs only)

Includes Exam, endorsement, retest, reinstatement, renewal, international, temp(endorsement)
Includes pending
Includes renewals
```{r}
state <- read_csv("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/WV-NPR Spreadsheet 1-1-19 to 10-7-21-clean.csv") %>% 
  clean_names()
...
#No removal necessary
```
Import Wyoming data (RNs only)

Includes exam, endorsement
No pending
No renewals
```{r}
state_end <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/WY-Credentials Issued.xlsx RN End.xlsx") %>% 
  clean_names() %>%
  mutate (lic_type = "RN",
...
state_exam <- read_excel("/Users/austinfast/Documents/NPR/210908-nursing licenses/State data/WY-Credentials Issued.xlsx RN Exam.xlsx", skip=1) %>% 
  clean_names() %>%
  mutate (lic_type = "RN",
          app_type = "Exam",
          application_date = mdy (application_submitted),
...
183 column names from 32 states pared down to 45 columns
```{r}
...
#How many have negative process times? 565,797 rows; 98% are renewal/reinstatement (555,127)
```{r}
all_data2 %>% 
...
#0.5% of data has negative process time and cannot be used -- input errors.
nrow(all_data2 %>% filter (process_time2 < 0) %>%
  filter (!(str_detect(app_type, "^Renewal")|str_detect(app_type, "^Reinstatement")))) / nrow(all_data2)  
...
#How much valid data do I have? 1.53M records (including pending); 1.43M without pending
#Adding CA renewals increases to 2.49M records (including pending); 2.43M without pending
all_data2 %>% 
  filter (process_time2 > -1) %>% 
...
#Louisiana has 294 records missing application dates that didn't calculate, rest of NAs are pending
all_data2 %>% 
  filter (is.na(process_time2) & year %in% c("2019", "2020", "2021")) %>%
  count (data_state)

#266 records (almost all NM) have 2022 issue dates, 244 renewals
all_data2 %>% 
  filter (year=="2022") #%>%
#  count (app_type)
...
Calculate averages for 32 states that provided usable data
```{r}
#1,549,715 records (without CA renewals) 
#1,945,411 for Jan. 1 - Sept. 23
...
#Majority are renewals (525793); 3125 = Endorsement and 2793 = Exam = 5918 removed
filtered_data %>% filter (process_time2 < 0) %>% count(standard_app_type) %>% View

#Four application types NPR is analyzing = 226,304 new licenses issued in 2021
filtered_data %>% filter (year=="2021" & standard_app_type %in% c("Exam", "Endorsement", "Other", "Unknown")) %>% count(standard_app_type) %>% View

#How many of these had negative process times that were removed? Just 77 licenses from six states
filtered_data %>% filter (year=="2021" & standard_app_type %in% c("Exam", "Endorsement", "Other", "Unknown") & process_time2 < 0) %>% count(standard_app_type, data_state) %>% View

#How many processing times are NA and will be removed? >> NONE
filtered_data %>% filter (is.na(process_time2)) %>% nrow()
...
#637,877 records removed (NA + negative)
...
#First states submitted licensees ending Sept 23, 2021.
#Later submissions include more weeks/months of records, that I'll filter out to standardize timeframe as Jan. 1, 2021-Sept. 23, 2021
```

```{r}
#removes 62,746 applications after Sept 23, 2021, when first states sent records
#removes 152,833 licenses issued after Sept 23, 2021
valid_data2 <- valid_data2 %>%
  filter (application_date <= as.Date("2021-09-23")) %>%
  filter (issue_date <= as.Date("2021-09-23")) 
#removes 155,237 total records, to end up with 1,366,109 records
...
#Keeps all new licenses, separating by RN and LPN only
...
#Keeps all new licenses, grouping RNs and LPNs and all application types together
#Removes exam-retest, reinstatements, renewals
#keeps only 2021
...
#Percent of new licenses issued that took longer than a month bumped from 78% in 2020 to 80% in 2021. 
#Percent of new licenses issued that took longer than 2 months jumped from 54% in 2020 to 58% in 2021. 
#Percent of new licenses issued that took longer than 3 months jumped from 32% in 2020 to 37% in 2021. 
...
```{r}
end_time <- Sys.time()
paste ("Process lasted", end_time - start_time)
```

START FROM HERE: Hit "Run All Chunks Above"

***COMPARISON***
Compare new licenses by year only, combining all nurses together to see total counts and percents that took over 30 days, 60 days, etc
```{r}
...
```{r}
#How many took over six months? 19,924 nurses
valid_data2 %>% 
...
#How many erroneous licenses removed?
valid
```


*** BIG FINDINGS ***
#Same percentages when filtering out those issued in Oct/Nov/Dec 2021
In these 32 states, over half (57%) the 226,000 RNs and LPNs issued new licenses Jan. 1-Sept. 23, 2021, waited more than two months (129,452 nurses). Over one-third  (35%) of nurses (80,056) waited more than 3 months to get a new license issued in 2021.

Count up totals by RN/LPN (new licenses only) - LPN numbers are slightly slower
```{r}
natl_averages %>%
  filter (lic_type %in% c("LPN", "RN")) %>%
  filter (year %in% c("2020", "2021")) %>%
...
#30 states that can provide endorsement counts (CT + VA cannot because lumped together)
state_averages %>%
  filter (lic_type %in% c("LPN", "RN")) %>%
  filter (year == "2021") %>%
...
#How many endorsement nurses licensed by these 30 states in 2021? (RE-RUN WITHOUT REMOVING AFTER 9-23-21)
105543 licensed by endorsement in 2021; 41343 pending (Jan 2021-Dec 2021) = 146886
Serious undercount since only includes 30 states, through September for most
```{r}
...
*** BIG FINDINGS *** 
#RNs slightly faster when filtering out those issued in Oct/Nov/Dec 2021
Longer than 60 days: 
* Exams: 63% of LPNs (13105) and 73% of RNs (67671) took longer than 60 days to process.
* Endorsement: 44% of LPNs (2593) and 43% of RNs (35429) took longer than 60 days to process.

Longer than 90 days: 
* Exams: 37% of LPNs (7797) and 44% of RNs (40545) took longer than 90 days to process.
* Endorsement: 32% of LPNs (1883) and 30% of RNs (24656) took longer than 90 days to process.

#Oklahoma, Texas and Maine - didn't provide any applications from 2019
#Most states didn't provide applications before 2019, meaning 2019 is incomplete
```{r}
...
*** BIG FINDINGS ***
Ranking states by license type and application type
...
What percent of American nurses are represented by these 30 states' data?
Approx 65% of nurse licenses nationally, using counts of active licenses as a proxy for new nurse applications
* Getting Florida would add 6%

Downloaded list of active RN/LPN licenses by state from the National Council of State Boards of Nursing, as of Nov. 23, 2021
https://www.ncsbn.org/14283.htm
https://www.ncsbn.org/Aggregate-AllActiveLicensesExcel.xls
```{r}
nnd <- read_excel ("/Users/austinfast/Documents/NPR/210908-nursing licenses/Aggregate-AllActiveLicensesExcel-112321.xls", skip=3) %>%
  select(1,4,7) %>%
...
nnd3 <- nnd2 %>%
  mutate (data = if_else (state %in% c("ar", "co", "ct", "hi", "ia", "il", "in", "ky", "la-pn", "ma", "me", "mi", "mn", "ms", "mt", "ne", "nc", "nh", "nj", "nm", "oh", "ok", "or", "pa", "sc", "tx", "tn", "vt", "va", "wv-rn", "wy", "ca-rn", "ca-pn"), "received", "missing"), #, "fl", 
...
Virginia claims to process 99% of applications within 30 days, but that's 30 days from getting marked complete.
docs_time records days from being marked complete. This confirms their claim, but also shows only
24.2% of the applications were issued within 30 calendar days from their submission.
```{r}
valid_data2 %>% 
...
* Ohio RN endorsement median increased from 63 days to 120 (57-day jump)
...
* NJ RN endorsement median decreased from 65 days to 50 (15-day drop)
* NJ LPN endorsement median decreased from 66 days to 53 (13-day drop)
* KY LPN endorsement median decreased from 44 days to 26 (18-day drop)
...
#Plot all types on one graph - MEDIAN
exam_end %>%
  ...
  labs(y="Median count of days", 
       ...
       caption="Source: State boards of nursing\n*CT & VA could not separate records by application type.",
       fill = "Type") +
...
Plot all types on one graph - MEAN  
```{r}
exam_end %>%
  mutate (type = paste0(standard_app_type, "-", lic_type)) %>%
  mutate (type = case_when ( #USE TO INCLUDE IN GRAPH BELOW
...
Export graphs of means/medians -- uses exam_end at line 4754 (before Connie Jin version)
```{r}
lic_types <- c("RN", "LPN")
app_types <- c("Exam", "Endorsement")
methods <- c("median", "mean")
...
for (license in lic_types){
  ...
  annotate("text", x=5, y=(natl_averages %>% filter (lic_type==license & standard_app_type==app & year=="2021") %>% pull(method) %>% as.numeric()), 
...
#Plot RN exam MEAN and MEDIAN
```{r}
exam_end %>%
  filter (lic_type=="RN" & standard_app_type=="Exam") %>%
  mutate (data_state = forcats::fct_reorder(data_state, mean_2021)) %>%
  ggplot(aes(x = data_state, y=mean_2021, fill=mean_2021)) +
  geom_bar(stat='identity', position="dodge")  +
  coord_flip() +
  #facet_wrap (~ data_state, ncol= 4, strip.position = "right") + 
  labs(y="Average count of days", 
       #x="State", 
       title="New RN licenses issued by Exam in 2021", 
       subtitle="Average days from application submission to license issuance", 
       caption="Source: State boards of nursing\n*CT & VA could not separate records by application type.",
       fill = "Type") +
  theme(axis.title.y=element_blank()) + 
  ...
  scale_fill_gradient2(low=scales::muted("darkgreen"), mid="yellow", high = "red", na.value = NA, 
                       #Set midpoint of color gradiant as national median for this type
                       midpoint=natl_averages %>% filter (lic_type=="RN" & standard_app_type=="Exam" & year=="2021") %>% pull(mean) %>% as.numeric()) + 
  theme(legend.position="none") +
  geom_hline(yintercept=(natl_averages %>% filter (lic_type=="RN" & standard_app_type=="Exam" & year=="2021") %>% pull(mean) %>% as.numeric()), linetype="dashed") +
  annotate("text", x=5, y=(natl_averages %>% filter (lic_type=="RN" & standard_app_type=="Exam" & year=="2021") %>% pull(mean) %>% as.numeric()), 
           label="National mean", size=3, angle=-90, vjust = -0.5)

#Plot RN exam MEDIAN  
#rn_exam_median <- 
exam_end %>%
  filter (lic_type=="RN" & standard_app_type=="Exam") %>%
  mutate (data_state = forcats::fct_reorder(data_state, median_2021)) %>%
  ggplot(aes(x = data_state, y=median_2021, fill=median_2021)) +
  geom_bar(stat='identity', position="dodge") +
  geom_text(aes(label = count_2021), position = position_stack(vjust=0.9), size=2.5) + #position_dodge(0.9)
  coord_flip() +
  #facet_wrap (~ data_state, ncol= 4, strip.position = "right") + 
  ...
  scale_fill_gradient2(low=scales::muted("darkgreen"), mid="yellow", high = "red", na.value = NA, 
                       #Set midpoint of color gradiant as national median for this type
                       midpoint=natl_averages %>% filter (lic_type=="RN" & standard_app_type=="Exam" & year=="2021") %>% pull(median) %>% as.numeric()) + 
  theme(legend.position="none") +
  geom_hline(yintercept=(natl_averages %>% filter (lic_type=="RN" & standard_app_type=="Exam" & year=="2021") %>% pull(median) %>% as.numeric()), linetype="dashed") +
  annotate("text", x=5, y=(natl_averages %>% filter (lic_type=="RN" & standard_app_type=="Exam" & year=="2021") %>% pull(median) %>% as.numeric()), 
           label="National median", size=3, angle=-90, vjust = -0.5) 
```

#reformatting for News Apps team & Connie Jin
```{r}
exam_end2 <- exam_end %>%
...
#Which states have other/unknown, meaning "All new licenses" will be more than sum of Exam+Endorsement?
state_averages %>%
  filter (year=="2021") %>%
...
How many new licenses in 2021? ##226575 (excluding re-tests [which skew medians longer], reinstatements, reactivations, etc.)
```{r}
avg_counts %>%
  ungroup() %>%
...
valid_data2 %>%
  ...
  summarize (total = n())
```
Create a histogram showing differences in counts and time to issuance for CALIFORNIA's deficient vs complete applications
```{r eval=FALSE}
...
```{r}
#How many of 2021's 69,806 applications were deficient? >> 29759
#55,550 exam/endorsements >> 28762 were deficient (19820 endorsement + 8942 exam)
ca_weeks_to_license2 %>% 
  ungroup() %>%
#  filter (year=="2021") %>%
  filter (standard_app_type %in% c("Exam", "Endorsement")) %>%
...
#Among all 2021's applications: (not just those licensed)
  #18728 RN endorsements were deficient (63% of all applications)
  #10835 (37% were complete)
  #1092 LPNs were deficient (68%)
  #506 LPNs were complete (32%)
  #Exams are better, probably because schools help with applications
  #40% of LPN exams complete
  #65% of RN exams complete

#removes 62,028 applications after Sept 23, 2021, when first states sent records
#removes 90,915 licenses issued after Sept 23, 2021
#This also removes all Pending licenses
...
#What percent of all licenses are deficient? 55% for whole year; 56% for Jan-Sept
ca_weeks_to_license5 %>%
  filter (standard_app_type %in% c("Exam", "Endorsement")) %>%
  group_by (deficient) %>%
  summarize (count = sum(count)) %>%
  ungroup() %>%
...
ca_deficient <- ca_weeks_to_license4 %>%
  rename (licensee_count = n) %>%
  filter (standard_app_type %in% c("Exam", "Endorsement")) %>%
  pivot_wider (id_cols = week_count,
               names_from = c(lic_type, standard_app_type, deficient),
               values_from = licensee_count) %>%
  arrange (week_count) %>%
  mutate (across(2:5, ~replace_na(.x, 0)))

#FOR CONNIE JIN's graphics
#write_csv (ca_deficient, "Nurse licenses-weeks to licensure-CA-deficient-FULLYEAR.csv")
ca_weeks_to_license5 <- ca_weeks_to_license5 %>%
  filter (lic_type=="RN")
#write_csv (ca_weeks_to_license5, "Nurse licenses-weeks to licensure-CA-medians.csv")
...
#How many licenses are cut off of chart
ca_weeks_to_license4 %>%
...
Create histograms of each state's number of weeks to license by license type and application type.
Save graphs of each
```{r eval=FALSE}
...
#  filter (standard_app_type == "Exam") %>%
 # filter (!(standard_app_type %in% c("Reinstatement", "Renewal", "Exam-retest"))) %>%
  ...
  facet_wrap (~ standard_app_type, ncol=1) + 
 # scale_x_continuous(breaks=c(0,30, 60, 90, 120)) +
  scale_x_continuous(breaks=seq(0, 53, by = 4.285714), #140 to reach max
                     labels=as.character(seq(0, 12, by = 1)), #32 to reach max
